...
class WegoApi(object):
        ...
        def get_wx_user(request, *args, **kwargs):
            ...
            if openid:
                ...
                if wx_user != 'error':
                    ...
                    if self.settings.REQUEST_HOOK:
                        self.settings.REQUEST_HOOK(request)
    ...
    def get_userinfo(self, helper, openid):
        ...
        wx_user = WeChatUser(self, data)
        if self.settings.UPSERT_USER:
            self.settings.UPSERT_USER(wx_user)
        return wx_user
    ...
    def _get_userinfo_from_session(self, helper):
        ...
        if self.settings.USERINFO_EXPIRE:
            ...
            if type(wx_userinfo) is bytes:
                wx_userinfo = wx_userinfo.decode()
            if wx_userinfo:
    ...
    # 获取 jsapi_ticket
    def get_jsticket(self, url, api_list, debug=True):

        # 缓存 ticket
        if not hasattr(self, 'jsapi_data') or self.jsapi_data['expires_at'] <= time.time():
            jsapi_data = self.wechat.get_jsticket()
            jsapi_data['expires_at'] = jsapi_data['expires_in'] + time.time()
            self.jsapi_data = jsapi_data

        jsapi_ticket = self.jsapi_data['ticket']

        noncestr = self._get_random_code()
        timestamp = int(time.time())
        enstr = 'jsapi_ticket=%s&noncestr=%s&timestamp=%s&url=%s' % (jsapi_ticket, noncestr, timestamp, url)
        enstr = enstr.encode()
    ...
    # 企业付款
    def company_pay(self, **kwargs):
        """
        get wechat config at https://pay.weixin.qq.com/wiki/doc/api/tools/mch_pay.php?chapter=14_2
        ...
        default_settings = {
            'mch_appid': self.settings.APP_ID,
            'mchid': self.settings.MCH_ID,
            'nonce_str': self._get_random_code(),
            'spbill_create_ip': self._get_local_ip()
        }
        ...
        data = self.wechat.company_pay(data)
...
class WeChatUser(object):
    ...
    def get_ext_userinfo(self):
        ...
        if self.wego.settings.UPSERT_USER:
            self.wego.settings.UPSERT_USER(self)
        return self.data
